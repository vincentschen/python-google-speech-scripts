...
BING_SPEECH_URL = 'https://api.datamarket.azure.com/Bing/speechrecognition/v1/'
...
def stt_bing(audio_fname):
    ...
    payload = {
        ...
        "appID":uuid.UUID('{D4D52672-91D7-4C74-8AD8-42B1D98141A5}'),
        "format":"json",
        "locale":"en-US",
        "device.os":"OSX",
        "scenarios":"ulm",
        "instanceid":uuid.uuid1().hex
    }
    
    r = requests.post(BING_SPEECH_URL, data=payload, headers=headers)
    print r.text
    
    # req = urllib2.Request(BING_SPEECH_URL, payload, headers)
    # print "Sending request to Bing"
    # 
    # try:
...
def stt_google_wav(audio_fname):
    ...
    if 'flac' not in filename:
        ...
        command_string = FLAC_CONV + ' ' + filename
    ...
    req = urllib2.Request(GOOGLE_SPEECH_URL, data=flac_cont, headers=hrs, debug=True)
